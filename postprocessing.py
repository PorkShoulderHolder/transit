...
test_connections = np.array([['901','L01'],
                    ['L01','723'],
                    ['723','635'],
                    ['635','L01'],
                    ['R31','L01']]).transpose()
...
test_costs = np.array(2 * [1, 2, 3, 4, 5])
test_prod = np.array([20, 10, -1, -4, -25])
test_nodes = np.array(['901', 'L01', '723', '635', 'R31'])
...
def create_edges_vector(edges):
    ...
    # ev = np.hstack((ev, test_connections)) #, np.vstack((test_connections[1], test_connections[0]))))
    nix = np.logical_not(np.all(ev == ev[0, :], axis=0))
    return ev[:, nix]


def create_cost_vector(edges):
    return edges.travel_time.values #, np.ones(len(test_connections.flatten()))))


def create_ids_vector(nodes):
    return nodes.parent_station.values #, test_connections[1]))
...
def add_transfer_edges(nodes, edges):
    ...
    def d(x):
        ...
        # print zip(same_trains.Id[ds < 400].values.tolist(), ds[ds < 400].values.tolist()), x.Id
